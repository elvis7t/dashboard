...
while($rs_user->GeraDados()){ ?> 
	<tr>
		<td><?=$rs_user->fld("usu_cod");?></td>
		<td><?=$rs_user->fld("emp_alias");?></td>
		<td><?=$rs_user->fld("dp_nome");?></td>
		<td><?=$rs_user->fld("usu_nome");?></td>
		<td><?=$rs_user->fld("classe_desc");?></td>
		<td> 
			<div class="button-group">
				<a 	class="btn btn-sm btn-primary" data-toggle='tooltip' data-placement='bottom' title='Alterar Usuário' a href="sys_alt_Usudes.php?token=<?=$_SESSION['token']?>&acao=N&usucod=<?=$rs_user->fld("usu_cod");?>"><i class="fa fa-pencil"></i></a>
