<?php
	...
	$sql ="SELECT * FROM eq_marca a
		...
		JOIN eq_tipo c ON c.tipo_id = a.marc_tipoId
			WHERE marc_empId=".$_SESSION['usu_empresa'];
	$rs->FreeSql($sql);
	if($rs->linhas==0):
		...
		<tr>
			...
			<td>
				<div class="button-group">
					<a 	class="btn btn-sm btn-primary" data-toggle='tooltip' data-placement='bottom' title='Alterar' a href="at_alt_Marcalocal.php?token=<?=$_SESSION['token']?>&acao=N&marcid=<?=$rs->fld('marc_id');?>"><i class="fa fa-pencil"></i></a>
