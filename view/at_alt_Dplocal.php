...
session_start();
$sess = "ATIVOLOCAL";
$pag = "at_departamentoslocais.php";// Fazer o link brilhar quando a pag estiver ativa
        ...
        <section class="content">
			<?php
				$menu = array(
							...
							"N" => array("class" => "btn btn-warning btn-sm", "icone" => "fa fa-refresh", "id"=>"btn_Altdp","label"=>"Alterar") 
							);
 				extract($_GET);
 				$rs = new recordset();
 				$sql ="SELECT * FROM at_departamentos a
				JOIN at_empresas b ON b.emp_id = a.dp_empid WHERE dp_id = ".$dpid;
 				$rs->FreeSql($sql);
 				$rs->GeraDados();
							...
							<div class="box-body">
								...
								<div id="clientes" class="row">
									<div class="form-group col-xs-1">
										...
										<input type="text" DISABLED class="form-control" name="dp_id" id="dp_id" value="<?=$rs->fld("dp_id");?>"/>
									...
									<div class="form-group col-xs-6 col-sm-4 col-md-4">  
										<label for="emp_nome">Departamento:</label>
											<div class="input-group">
										<div class="input-group-addon">
											<i class="glyphicon glyphicon-tasks"></i>
											</div> 
										<input type="text"  class="form-control" name="dp_nome" id="dp_nome" value="<?=$rs->fld("dp_nome");?>"/>
									</div>
									</div>
