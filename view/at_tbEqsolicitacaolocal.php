...
while($rs->GeraDados()){ ?> 
	<tr> 
		...
		<td><?echo $fn->data_hbr($rs->fld("solic_data"));?></td>  	
		<td><?=$rs->fld("usu_nome");?></td> 
		<td><?=$rs->fld("solic_ticket");?></td>  
		<td>     
			<div class="button-group">
				<!--<a 	class="btn btn-xs btn-primary" data-toggle='tooltip' data-placement='bottom' title='Alterar' a href="at_alt_Sollocal.php?token=<?=$_SESSION['token']?>&acao=N&solid=<?=$rs->fld('solic_id');?>"><i class="fa fa-pencil"></i></a>-->
				<a 	class="btn btn-sm btn-warning"
						data-toggle='tooltip' 
						data-placement='bottom'     
						title='Detalhes'
						a href="at_ver_Solicitacaolocal.php?token=<?=$_SESSION['token']?>&acao=N&solid=<?=$rs->fld('solic_id');?>"><i class="fa fa-search"></i> 
					</a>   
			</div>
		</td> 
